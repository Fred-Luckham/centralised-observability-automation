...
ReputationBounceRateP1 = os.environ['ReputationBounceRateP1']
ReputationComplaintRateP1 = os.environ['ReputationComplaintRateP1']

def lambda_handler(event, context):
  try:
    logger.info(f'event: {event}')
    session = assume_role(event)
    create_reputation_bounce_rate_alarm(event, session)
    create_reputation_complaint_rate_alarm(event, session)
    event['alarms'] = alarms
    return event
...
def create_reputation_bounce_rate_alarm(event, session):
  alarm_object = Alarm(
    [["Critical", ReputationBounceRateP1, "P1"]], 
    ...
    "Average", 
    True, 
    "Alarm triggers when reputation bounce rate exceeds  0.02%", 
    "notBreaching", 
    [{'Name': 'FunctionName','Value': event['instance_id']}]
  )
  for_each_threshold(event, session, alarm_object)

def create_reputation_complaint_rate_alarm(event, session):
  alarm_object = Alarm(
    [["Critical", ReputationComplaintRateP1, "P1"]], 
    "GreaterThanThreshold", 
    EvaluationPeriods, 
    ...
    "Average", 
    True, 
    "Alarm triggers when reputation bounce rate exceeds  0.004%", 
    "notBreaching", 
    [{'Name': 'FunctionName','Value': event['instance_id']}]
  )
  for_each_threshold(event, session, alarm_object)
